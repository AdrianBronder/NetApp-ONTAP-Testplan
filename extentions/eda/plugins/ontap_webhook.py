...
fron NetApp ONTAP systems. Copy of the original EDA webhook.py module.
The message is accepted in JSON as well as XML format (default for ONTAP
is currently XML).
...
async def ontap_webhook(request: web.Request) -> web.Response:
    """Return response to ontap_webhook request."""
...
async def main(queue: asyncio.Queue, args: dict[str, Any]) -> None:
    """Receive events via ontap_webhook."""
    if "port" not in args:
